...
function ranklinear(sp::Real)
    ...
    function rank(fitness::Vector{<:Real}, N::Int; rng::AbstractRNG = Random.GLOBAL_RNG)
        λ = length(fitness)
...
"""
function roulette(fitness::Vector{<:Real}, N::Int; rng::AbstractRNG = Random.GLOBAL_RNG)
    absf = abs.(fitness)
    prob = absf ./ sum(absf)
...
"""
function sus(fitness::AbstractMatrix, N::Int; rng::AbstractRNG = Random.GLOBAL_RNG)
    F = sum(abs, fitness)
    P = F / N
    start = P * rand(rng)
...
function tournament(groupSize::Int; select = argmin)
    ...
    function tournamentN(
        fitness::AbstractVecOrMat{<:Real},
        N::Int;
