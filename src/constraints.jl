...
PenaltyConstraints(penalty::T, bounds::ConstraintBounds{T}, cf = (x) -> nothing) where {T} =
    PenaltyConstraints(
        fill(penalty, nconstraints(bounds) + nconstraints_x(bounds)),
        bounds,
        cf,
    )
PenaltyConstraints(
    penalty::AbstractVector{T},
    lx::AbstractVector{T},
    ux::AbstractVector{T},
    lc::AbstractVector{T},
    uc::AbstractVector{T},
    cf = (x) -> nothing,
) where {T} = PenaltyConstraints(penalty, ConstraintBounds(lx, ux, lc, uc), cf)
PenaltyConstraints(
    penalty::T,
    lx::AbstractVector{T},
    ux::AbstractVector{T},
    lc::AbstractVector{T} = T[],
    uc::AbstractVector{T} = T[],
    cf = (x) -> nothing,
) where {T} = PenaltyConstraints(penalty, ConstraintBounds(lx, ux, lc, uc), cf)
PenaltyConstraints(
    lx::AbstractVector{T},
    ...
    uc::AbstractVector{T} = T[],
    cf = (x) -> nothing,
) where {T} = PenaltyConstraints(one(T), ConstraintBounds(lx, ux, lc, uc), cf)
...
function penalty!(
    fitness::AbstractVector{T},
    c::PenaltyConstraints{T,F},
    population,
) where {T,F}
...
WorstFitnessConstraints(
    ...
    uc::AbstractVector{T},
    cf = (x) -> nothing,
) where {T} = WorstFitnessConstraints(ConstraintBounds(lx, ux, lc, uc), cf)
...
penalty!(fitness, c::MixedTypePenaltyConstraints, population) =
    penalty!(fitness, c.penalty, population)
...
function penalty(
    ...
    x,
    c::Union{AbstractVector{T},Nothing} = nothing,
) where {T}
    penalty = 0
